...
ems_hourlyustfiltnongf_2023  <- ems_hourlyustfiltnongf_2023  %>% filter(ems_hourlyustfiltnongf_2023 $month > 5)
ems_hourlyustfiltnongf_2023  <- ems_hourlyustfiltnongf_2023  %>% filter(ems_hourlyustfiltnongf_2023 $month <10)
ems_hourlyustfiltnongf_2023  <- ems_hourlyustfiltnongf_2023  %>% filter(ems_hourlyustfiltnongf_2023 $year >2016)
...
Ha1_ETfill_9222  <- Ha1_ETfill_9222  %>% filter(Ha1_ETfill_9222 $USTAR >0)
...
colnames(Ha1_ETfill_9222)[colnames(Ha1_ETfill_9222) == "DateTime"] <- "datetime"
...
LCL <- merge(ems_hourlyustfiltnongf_2023,df_anom,by="datetime")
...
LCL$RH
#still need LCL
LCL_clean <- LCL[,c("datetime","VSWCAnom.mean.med","LE","RH","hpbl","hour","PA","Tair","H")]
#LCL_clean <- LCL[,c("datetime","VSWCAnom.mean.med","RH_PI_F_1_2_1","hpbl","hour","PA_PI_F","TA_PI_F_1_2_1")]

#LCL input should have no Nans, RH should be 1 maximum

LCL_clean$RH <- replace(LCL_clean$RH,LCL_clean$RH>100,100)



LCL_clean <- LCL_clean[!is.na(LCL_clean$RH), ]
LCL_clean <- LCL_clean[!is.na(LCL_clean$Tair), ]
LCL_clean <- LCL_clean[!is.na(LCL_clean$PA), ]
...
LCL_clean$LCL <- lcl(p=LCL_clean$PA*1000, T=LCL_clean$Tair+273.15,rh=LCL_clean$RH/100)
LCL_clean <- na.omit(LCL_clean)

LCL_clean$LCL <- lcl(p=LCL_clean$PA_PI_F*1000, T=LCL_clean$TA_PI_F_1_1_1+273.15,rh=LCL_clean$RH_PI_F_1_1_1/100)
...
quartiles <- quantile(LCL_clean$VSWCAnom.mean.med, probs=c(.1, .9), na.rm = TRUE)
summary(LCL_clean$VSWCAnom.mean.med)

LCL_DRY <- LCL_clean %>% filter(VSWCAnom.mean.med < -.0419)

LCL_WET <- LCL_clean %>% filter(VSWCAnom.mean.med > .0285)
...
#either soil moisture issue or issue of which Ha1 level was used for RH and temp
